...
static unsigned int const DEFAULT_REDIRECTS = 10;
...
[[gnu::constructor]]
static void AsyncHTTPClientBridgeEnsureObjFWTLSBindingsLoaded(void)
{
    volatile int objfwTLSReference = _ObjFWTLS_reference;
    (void)objfwTLSReference;
}

static void AsyncHTTPClientBridgeInitialiseState(void)
{
...
@implementation AsyncHTTPClientBridgeException
...
- (OFString *)description
{
    return [OFString stringWithFormat: @"AsyncHTTPClientBridgeException: %@", self.reason];
}
...
+ (void)retainInflightBridge: (AsyncHTTPClientTaskBridge *)bridge
{
    OFOnce(&async_http_client_bridge_once, AsyncHTTPClientBridgeInitialiseState);
...
- (OFException *)_exceptionFromCallbackException: (id)exception
{
    ...
    return [[AsyncHTTPClientBridgeException alloc] initWithReason: @"HTTP client failed with a non-OFException value"];
}
...
- (void)_finishWithResponse: (OFHTTPResponse *nillable)response exception: (id nillable)exception
{
    ...
    @try {
        ...
        if (response == nilptr) {
            auto missingResponseException = [[AsyncHTTPClientBridgeException alloc] initWithReason: @"HTTP client completed without a response"];
...
@implementation OFHTTPClient (AsyncHTTPClientBridge)
                                         ...
                                         onScheduler: (AsyncScheduler *)scheduler
{
    ...
    auto bridge = [[AsyncHTTPClientTaskBridge alloc] initWithRequestClient: self
