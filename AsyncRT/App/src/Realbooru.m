...
{
    return [Task rejected: [[RealbooruAPIException alloc] initWithReason: @"Realbooru tag listing is unavailable because Realbooru's DAPI is offline"
                                                     underlyingException: nilptr]];
}

- (Task<Optional<BooruPage *> *> *)fetchPage: (int)pageNumber forSearchWithTags: (OFArray<OFString *> *)tags
{
    return [self fetchPage: pageNumber forSearchWithTags: tags excludingTags: @[]];
}

- (Task<Optional<BooruPage *> *> *)fetchPage: (int)pageNumber
                               ...
                               excludingTags: (OFArray<OFString *> *)excludedTags
{
    ...
    if (pageNumber < 0)
        @throw [OFInvalidArgumentException exception];
    if (taskGroup == nilptr)
        return [Task rejected: [[RealbooruAPIException alloc] initWithReason: @"fetchPage requires an active AsyncTaskGroup"
                                                         underlyingException: nilptr]];
...
- (Task<OFString *> *)_fetchHTMLAtIRI: (OFIRI *)IRI onScheduler: (AsyncScheduler *)scheduler [[direct]]
{
    ...
    return [[_httpClient taskToPerformHTTPRequest: request onScheduler: scheduler] mapOnScheduler: scheduler transform: ^OFString *(OFHTTPResponse *response) {
        if (response.statusCode < 200 or response.statusCode >= 300)
            @throw [[RealbooruAPIException alloc] initWithReason: [OFString stringWithFormat: @"Realbooru returned HTTP status %hd", response.statusCode]
                                             underlyingException: nilptr];
...
{
    return [self _IRIWithQueryItems: @[
