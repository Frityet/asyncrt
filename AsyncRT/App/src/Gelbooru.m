...
- (Task<OFArray<OFString *> *> *)fetchAllTags
{
    ...
    if (taskGroup == nilptr)
        return [Task rejected: [[GelbooruAPIException alloc] initWithReason: @"fetchAllTags requires an active AsyncTaskGroup"
                                                        underlyingException: nilptr]];

    AsyncTaskGroup *activeTaskGroup = $assert_nonnil(taskGroup);

    return (Task<OFArray<OFString *> *> *)[activeTaskGroup spawnTask: ^id {
                               ...
                               excludingTags: (OFArray<OFString *> *)excludedTags
{
    ...
    if (pageNumber < 0)
        @throw [OFInvalidArgumentException exception];
    if (taskGroup == nilptr)
        return [Task rejected: [[GelbooruAPIException alloc] initWithReason: @"fetchPage requires an active AsyncTaskGroup"
                                                        underlyingException: nilptr]];
...
- (Task<id> *)_fetchJSONAtIRI: (OFIRI *)IRI onScheduler: (AsyncScheduler *)scheduler [[direct]]
{
    auto request = [[OFHTTPRequest alloc] initWithIRI: IRI];
    request.headers = @{ @"User-Agent": @"BooruAggr/1.0" };

    return [[_httpClient taskToPerformHTTPRequest: request onScheduler: scheduler] mapOnScheduler: scheduler transform: ^id(OFHTTPResponse *response) {
