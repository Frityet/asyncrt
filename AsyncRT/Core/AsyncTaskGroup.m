...
@implementation AsyncTaskGroup {
    OFMutex *_lock;
    OFMutableSet<AsyncTask *> *_liveTasks;
...
- (instancetype)initWithScheduler: (AsyncScheduler *)scheduler ownerTask: (AsyncTask *)ownerTask parentTaskGroup: (AsyncTaskGroup *nillable)parentTaskGroup name: (OFString *nillable)name deadline: (OFDate *nillable)deadline
{
    self = [super init];
...
- (void)_recordFailureIfNeeded: (OFException *)exception
{
    ...
    if ([exception isKindOfClass: AsyncTaskCancelledException.class])
        return;
...
- (void)_cancelOwnedTasks
{
    ...
    for (AsyncTask *task in liveTasks)
        [task cancel];
}
...
- (AsyncTask<id> *)spawnTask: (id (^)(void))block
{
    return [self spawnTask: block name: nilptr];
}

- (AsyncTask<id> *)spawnTask: (id (^)(void))block name: (OFString *nillable)name
{
    ...
    return [[AsyncTask alloc] initWithScheduler: self.scheduler taskGroup: self name: name block: block];
}

- (AsyncTask<id> *)spawnTaskInChildTaskGroup: (id (^)(AsyncTaskGroup *taskGroup))block name: (OFString *nillable)name
{
    AsyncTask *currentTask = AsyncTask.currentTask;
...
- (AsyncTask<id> *)spawnTaskInChildTaskGroup: (id (^)(AsyncTaskGroup *taskGroup))block
{
    return [self spawnTaskInChildTaskGroup: block name: nilptr];
}

- (AsyncTask<OFArray<id> *> *)spawnAllTasks: (OFArray<id (^)(void)> *)blocks
{
    return [self spawnAllTasks: blocks name: nilptr];
}

- (AsyncTask<OFArray<id> *> *)spawnAllTasks: (OFArray<id (^)(void)> *)blocks name: (OFString *nillable)name
{
    OFArray<id (^)(void)> *taskBlocks = [blocks copy];

    return (AsyncTask<OFArray<id> *> *)[self spawnTask: ^{
        auto tasks = [OFMutableArray<AsyncTask *> arrayWithCapacity: taskBlocks.count];
        ...
        return [AsyncTask all: tasks].await;
    } name: name];
}
...
{
    auto currentTask = AsyncTask.currentTask;
    if (currentTask == nilptr)
        @throw [OFInvalidArgumentException exception];
    if (currentTask.scheduler != self.scheduler)
        @throw [OFInvalidArgumentException exception];
...
- (id)_runTaskGroupBody: (id (^)(AsyncTaskGroup *taskGroup))block
{
    ...
    if (bodyException != nilptr and not [bodyException isKindOfClass: AsyncTaskCancelledException.class]) {
