...
@private
    unretained AsyncUICocoaWindow *_backend;
...
- (instancetype)initWithBackend: (AsyncUICocoaWindow *)backend frame: (NSRect)frameRect;
...
- (AsyncImmediateUIApplication *nillable)ui_application
{
...
{
    AsyncImmediateUIApplication *application;
...
{
    AsyncImmediateUIApplication *application = self.ui_application;
...
[[direct_members]]
@interface AsyncUICocoaWindow ()<AsyncUINativeWindowDelegate>
...
[[direct_members]]
@implementation AsyncUICocoaWindow {
    bool _open;
...
- (void)openWindow
{
    ...
    if (not NSThread.isMainThread)
        @throw [[AsyncUIInitializationException alloc] initWithReason: @"AsyncUICocoaWindow must be used on the main thread"];
