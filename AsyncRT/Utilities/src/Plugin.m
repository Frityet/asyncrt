...
+ (bool)_class: (Class)cls implementsProtocol: (Protocol *)protocol;
...
+ (bool)_class: (Class)cls implementsProtocol: (Protocol *)protocol
{
    for (Class currentClass = cls; currentClass != nullptr; currentClass = class_getSuperclass(currentClass)) {
...
- (instancetype)initWithPath: (OFString *)path
{
    ...
    _path = [path copy];
    Class moduleClass = [self.class _moduleClassForPluginPath: _path];
    OFData *classesBeforeLoad = [self.class _allLoadedClassPointers];
...
#pragma clang diagnostic pop
    ...
    _module = module;
    _loadedClassPointers = [self.class _classPointersIn: self.class._allLoadedClassPointers  excluding: classesBeforeLoad];
    return self;
}
...
- (OFArray<Class> *)classesThatImplementProtocol: (Protocol *)protocol
{
    OFData *classPointers = (_isCurrentProcess ? self.class._allLoadedClassPointers : _loadedClassPointers);
    auto classes = (Class unretained const *)classPointers.items;
    auto matchingClasses = [OFMutableArray<Class> array];

    for (size_t classIndex = 0; classIndex < classPointers.count; classIndex++) {
        Class cls = classes[classIndex];

        if ([self.class _class: cls implementsProtocol: protocol])
            [matchingClasses addObject: cls];
    }

    return [matchingClasses copy];
}
