...
@implementation CXXBindTemporaryExpr
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    auto dictionary = $cast(OFDictionary, obj);
    {
        ...
        if (value != nilptr)
            _dtor = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"temp"];
        _temp = $cast(OFString, $assert_nonnil(value));
    }
    {
