...
@implementation InlineCommandComment
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"args"];
        if (value != nilptr)
            _args = $cast(OFArray, value);
    }
    {
    ...
    {
        auto value = dictionary[@"renderKind"];
        _renderKind = $cast(OFString, $assert_nonnil(value));
    }
