...
@implementation TypeNodeShape007
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"numElements"];
        _numElements = $cast(OFNumber, $assert_nonnil(value));
    }
    {
    ...
    {
        auto value = dictionary[@"vectorKind"];
        if (value != nilptr)
            _vectorKind = $cast(OFString, value);
    }
