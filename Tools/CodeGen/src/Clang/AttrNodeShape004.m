...
@implementation AttrNodeShape004
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"message"];
        if (value != nilptr)
            _message = $cast(OFString, value);
    }
    {
    ...
    {
        auto value = dictionary[@"replacement"];
        if (value != nilptr)
            _replacement = $cast(OFString, value);
    }
