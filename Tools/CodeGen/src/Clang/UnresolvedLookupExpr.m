...
@implementation UnresolvedLookupExpr
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"lookups"];
        auto array = $cast(OFArray, $assert_nonnil(value));
        auto converted = [OFMutableArray arrayWithCapacity: array.count];
        for (id item in array)
            [converted addObject: [[BareDeclRef alloc] initFromJSONObject: item]];
        _lookups = [converted copy];
    }
    {
    ...
    {
        auto value = dictionary[@"usesADL"];
        _usesADL = [$cast(OFNumber, $assert_nonnil(value)) boolValue];
    }
    {
