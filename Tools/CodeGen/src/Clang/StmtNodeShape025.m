...
@implementation StmtNodeShape025
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"array_filler"];
        if (value != nilptr) {
            ...
            _array_filler = [converted copy];
        }
    }
    {
        auto value = dictionary[@"field"];
        if (value != nilptr)
            _field = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
