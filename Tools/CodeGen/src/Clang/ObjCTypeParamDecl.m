...
@implementation ObjCTypeParamDecl
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"bounded"];
        if (value != nilptr)
            _bounded = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"variance"];
        if (value != nilptr)
            _variance = $cast(OFString, value);
    }
