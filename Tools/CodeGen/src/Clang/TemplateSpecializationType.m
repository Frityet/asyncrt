...
@implementation TemplateSpecializationType
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"isAlias"];
        if (value != nilptr)
            _isAlias = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"templateName"];
        _templateName = $cast(OFString, $assert_nonnil(value));
    }
    {
