...
@implementation TemplateTemplateParmDecl
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"defaultArg"];
        if (value != nilptr)
            _defaultArg = [[TemplateArgument alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
        auto value = dictionary[@"depth"];
        _depth = $cast(OFNumber, $assert_nonnil(value));
    }
    {
