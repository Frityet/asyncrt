...
@implementation ObjCAtDefsFieldDecl
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"hasInClassInitializer"];
        if (value != nilptr)
            _hasInClassInitializer = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"isBitfield"];
        if (value != nilptr)
            _isBitfield = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"mutable"];
        if (value != nilptr)
            _jsonMutable = [$cast(OFNumber, value) boolValue];
    }
    {
