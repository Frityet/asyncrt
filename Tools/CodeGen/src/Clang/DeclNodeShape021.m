...
@implementation DeclNodeShape021
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"implementation"];
        _implementation = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"interface"];
        _interface = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"protocols"];
        if (value != nilptr) {
            auto array = $cast(OFArray, value);
            auto converted = [OFMutableArray arrayWithCapacity: array.count];
            for (id item in array)
                [converted addObject: [[BareDeclRef alloc] initFromJSONObject: item]];
            _protocols = [converted copy];
        }
    }
    {
