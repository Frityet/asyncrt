...
@implementation DeclNodeShape002
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"isInline"];
        if (value != nilptr)
            _isInline = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"isNested"];
        if (value != nilptr)
            _isNested = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"originalNamespace"];
        if (value != nilptr)
            _originalNamespace = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
