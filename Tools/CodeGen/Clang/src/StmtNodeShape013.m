...
@implementation StmtNodeShape013
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"foundReferencedDecl"];
        if (value != nilptr)
            _foundReferencedDecl = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"nonOdrUseReason"];
        if (value != nilptr)
            _nonOdrUseReason = $cast(OFString, value);
    }
    {
    ...
    {
        auto value = dictionary[@"referencedDecl"];
        _referencedDecl = [[BareDeclRef alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
