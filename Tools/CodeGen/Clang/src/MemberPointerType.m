...
@implementation MemberPointerType
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"isData"];
        if (value != nilptr)
            _isData = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"isFunction"];
        if (value != nilptr)
            _isFunction = [$cast(OFNumber, value) boolValue];
    }
    {
