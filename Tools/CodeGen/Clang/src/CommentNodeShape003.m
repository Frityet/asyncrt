...
@implementation CommentNodeShape003
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"attrs"];
        if (value != nilptr)
            _attrs = $cast(OFArray, value);
    }
    {
    ...
    {
        auto value = dictionary[@"malformed"];
        if (value != nilptr)
            _malformed = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"selfClosing"];
        if (value != nilptr)
            _selfClosing = [$cast(OFNumber, value) boolValue];
    }
