...
@implementation ArrayParameterType
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"indexTypeQualifiers"];
        if (value != nilptr)
            _indexTypeQualifiers = $cast(OFString, value);
    }
    {
    ...
    {
        auto value = dictionary[@"size"];
        _size = $cast(OFNumber, $assert_nonnil(value));
    }
    {
        auto value = dictionary[@"sizeModifier"];
        if (value != nilptr)
            _sizeModifier = $cast(OFString, value);
    }
    {
