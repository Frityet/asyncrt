...
@implementation TypeNodeShape013
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"isTagOwned"];
        if (value != nilptr)
            _isTagOwned = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"qualifier"];
        if (value != nilptr)
            _qualifier = $cast(OFString, value);
    }
    {
