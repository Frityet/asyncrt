...
@implementation StmtNodeShape029
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"adjustedTypeArg"];
        if (value != nilptr)
            _adjustedTypeArg = [[JSONQualType alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"typeArg"];
        if (value != nilptr)
            _typeArg = [[JSONQualType alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
