...
@implementation StmtNodeShape017
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"canOverflow"];
        if (value != nilptr)
            _canOverflow = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"isPostfix"];
        _isPostfix = [$cast(OFNumber, $assert_nonnil(value)) boolValue];
    }
    {
