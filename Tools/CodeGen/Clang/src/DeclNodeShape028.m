...
@implementation DeclNodeShape028
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"hasBraces"];
        if (value != nilptr)
            _hasBraces = [$cast(OFNumber, value) boolValue];
    }
    {
    ...
    {
        auto value = dictionary[@"language"];
        _language = $cast(OFString, $assert_nonnil(value));
    }
    {
