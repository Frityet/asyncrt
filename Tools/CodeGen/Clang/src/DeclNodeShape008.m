...
@implementation DeclNodeShape008
...
- (instancetype)initFromJSONObject: (id)obj
{
    ...
    {
        auto value = dictionary[@"fixedUnderlyingType"];
        if (value != nilptr)
            _fixedUnderlyingType = [[JSONQualType alloc] initFromJSONObject: $assert_nonnil(value)];
    }
    {
    ...
    {
        auto value = dictionary[@"scopedEnumTag"];
        if (value != nilptr)
            _scopedEnumTag = $cast(OFString, value);
    }
