...
@implementation OFSPXStreamSocket (FutureAdditions)
...
{
    if ((OFSPXStreamSocket *nillable)self == nilptr or (const unsigned char *nillable)node == nullptr or (AsyncScheduler *nillable)scheduler == nilptr)
    ...
    FutureResolver<OFSPXStreamSocket *> *resolver = [[FutureResolver alloc] init];
    AsyncObjFWFutureBridge *bridge = [[AsyncObjFWFutureBridge alloc] initWithObject: self operation: @"asyncConnectToNetwork:node:port:" scheduler: scheduler resolver: (FutureResolver<id> *)resolver startBlock: ^(AsyncObjFWFutureBridge *bridge) {
        [self asyncConnectToNetwork: network node: expectedNodePointer port: port runLoopMode: scheduler.mode handler: ^(OFSPXStreamSocket *socket, uint32_t callbackNetwork, const unsigned char callbackNode[_Nonnull IPX_NODE_LEN], uint16_t callbackPort, id nillable exception) {
            if (exception != nilptr) {
            ...
            if (socket != self or callbackNetwork != network or memcmp(callbackNode, expectedNodePointer, IPX_NODE_LEN) != 0 or callbackPort != port) {
                [bridge rejectInvalidCompletionWithReason: @"ObjFW completed an SPX stream connect with mismatched connection metadata"];
                return;
            }
